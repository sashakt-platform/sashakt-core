...
def get_org_role(session: Session, organization_id: int, role_name: str) -> Role:
    """
...
def create_random_role(session: Session, organization_id: int | None = None) -> Role:
    if organization_id is None:
        organization = create_random_organization(session=session)
    ...
    db_role = Role(
        **role_in.model_dump(exclude={"permissions", "visible_to_roles"}),
