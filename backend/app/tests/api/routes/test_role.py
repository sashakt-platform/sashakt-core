...
def test_read_role(
    ...
    assert content["is_active"] is None
...
def test_read_role_not_found(
    ...
    content = response.json()
    assert re.search(r"\bnot\b.*\bfound\b", content["detail"])
...
def test_read_role_not_enough_permissions(
    ...
    role = create_random_role(db)
    print("Random role is ", role)
    print("normal_user_token_headers->", normal_user_token_headers)
...
def test_delete_role(
    ...
    assert response.status_code == 200
    content = response.json()
    assert re.search(r"delete", content["message"])
