...
def test_submit_test_auto_submit_skips_mandatory_validation(
    client: TestClient, db: SessionDep
) -> None:
    """Test that auto_submit=True skips mandatory validation (for timeout scenarios)."""
    user = create_random_user(db)
    ...
    # Submit with auto_submit=True without answering mandatory question - should succeed
    response = client.post(
        f"{settings.API_V1_STR}/candidate/submit_test/{candidate_test_id}",
        params={"candidate_uuid": candidate_uuid, "auto_submit": True},
    )
